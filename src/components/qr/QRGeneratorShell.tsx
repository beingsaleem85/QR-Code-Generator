...
import { AUTH_REQUIRED, TRIAL_EXPIRED } from "@/lib/qr/action-types";
import { saveQrCode, updateQrCode } from "@/lib/qr/actions";
import { clearDraft, stashDraft } from "@/lib/qr/draft-storage";
...
interface QRGeneratorShellProps {
  ...
  /** True when the Free account's 14-day trial has expired — blocks creating new QRs. */
  isTrialExpired?: boolean;
  /** Explicit authentication status if known (e.g. from server component) */
...
export function QRGeneratorShell({
  ...
  const effectiveAuthStatus =
    isAuthenticated !== undefined
      ...
      : variant === "edit"
        ? true
        : authStatus;
  const effectiveTrialExpired = isTrialExpired || trialExpired;
  const savingRef = useRef(false);

  useEffect(() => {
    if (isAuthenticated !== undefined || variant === "edit") {
      return;
    }
    try {
      ...
      // Ignore if supabase client is not initialized in test env
    }
  ...
  const triggerGuestAction = () => {
    stashDraft({
      name: name.trim() || "My QR code",
      mode,
      qrType,
      content,
      design,
  ...
  const handleSave = async () => {
    if (!qrCodeId && (isTrialExpired || trialExpired)) {
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
        <div className="flex flex-col gap-4">
          <QRContentPanel
            ...
            onChange={setContent}
          />
          <QRDesignPanel qrType={qrType} value={design} onChange={setDesign} />
        </div>
        ...
        <div className="flex flex-col gap-4 lg:sticky lg:top-20 lg:self-start">
          <QRPreviewPanel
            ...
            isAuthenticated={effectiveAuthStatus ?? true}
            isTrialExpired={effectiveTrialExpired}
            onGuestAction={triggerGuestAction}
