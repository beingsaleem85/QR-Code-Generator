...
- **Current module:** 3.5 — Saving and Managing QR Codes (not yet started)
- **Last completed module:** 3.4 — QR Download and Export (COMPLETE)
- **Branch/commit:** local git repo (`master`). Check `git log --oneline -20` for the actual latest commit when resuming — as of writing, Modules 1.1–3.3 are committed and Module 3.4 is ready to commit.
- **Supabase integration status:** **Live and connected.** Project URL + anon key are in `.env.local` (gitignored). The live project's schema (all 8 migrations) has been pushed and matches `supabase/migrations/`. Real signup/login/logout/session-persistence/password-recovery/profile-upsert are all wired and were verified end-to-end against the live project (see `docs/WORKLOG.md`'s Module 3.1 entry) — this is no longer mock data for auth. **Nothing QR-related is persisted yet** — Module 3.5 is the first module to write to `qr_codes`.
- **QR rendering status:** Fully real, fully styled, and fully exportable. `src/lib/qr/matrix.ts` + `styled-svg.ts` render every design control via a custom per-module SVG renderer; `reliability.ts` implements contrast/logo-size/quiet-zone/EC-level safeguards; logo upload is real (client-side data URL); downloads support 3 PNG resolution presets + vector SVG with a complete filename-sanitization policy (`slugifyForFilename()`, `src/lib/qr/render.ts`). "Save QR" is still the one disabled button left in `QRDownloadActions` — that's Module 3.5.
- **Test status:** 168 unit/component tests passing. `typecheck`, `lint` (0 errors), `format:check`, and a fresh production `build` (25 routes) all pass.
...
None. Module 3.5 doesn't need anything from the user to start.
...
Start Module 3.5 (Saving and Managing QR Codes) — re-read its full section in `QR_Code_Generator_Master_Build_Prompt.md` first. This is the first module that writes to `qr_codes`, and it touches a lot of already-built UI that's currently mock-data-driven:

1. **Server-side CRUD, RLS-respecting**: `qr_codes` already exists with RLS policies scoped to `auth.uid()` (Module 1.4/1.5) — use `src/lib/supabase/server.ts`'s server client from Server Actions/Route Handlers, never trust client-supplied ownership. Persistence rules from the master prompt: store QR type, mode, payload config (the validated `content` object), design config (`DesignConfig`, already fully real as of Module 3.3), destination, asset references, timestamps — explicitly do **not** store generated base64 images in Postgres (regenerate visuals from saved config via the existing `styled-svg.ts` renderer instead — this is exactly why `design_config` stores a data-URL logo but never a rendered QR image itself).
2. **Save QR** — `QRDownloadActions`' "Save QR" button (`src/components/qr/QRDownloadActions.tsx`) is the last remaining disabled control in the generator; wire it to a real create/update call. `QRGeneratorShell`'s `variant="edit"` "Save Changes" button (Module 2.7) needs the same treatment for editing an existing saved QR.
3. **Replace mock data with real queries**, one consumer at a time: `src/lib/qr/mock-data.ts`'s `MOCK_QR_CODES`/`findMockQrCode()` currently backs the dashboard overview (`/dashboard`), the QR codes list (`/dashboard/qr-codes`), the detail page (`/dashboard/qr-codes/[id]`), and the edit page (`/dashboard/qr-codes/[id]/edit`) — all four need real Supabase queries scoped to the signed-in user.
4. **Duplicate, archive, delete-with-confirmation**: the QR detail page (Module 2.7) already has a "Danger zone" with disabled Archive/Delete buttons — wire them for real, with a real confirmation step for delete. "Duplicate" has no existing UI placeholder — decide where it belongs (likely the detail page, alongside Edit) and add it.
5. **Download again**: once a QR is saved and later revisited, its detail page's (currently disabled) download buttons should work the same way `QRDownloadActions` already does — regenerate the image from the saved `design_config`/payload rather than storing one.
6. Document in `docs/ARCHITECTURE.md`, mark Module 3.5 complete in `docs/WORKLOG.md`, commit. Continue autonomously into Module 3.6 (Dynamic QR Codes) afterward per the standing instruction, unless a blocking need arises.
