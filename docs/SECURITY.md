...
- 5 small orphaned Storage blobs remain from Module 3.16's E2E test fixtures — harmless, isolated per-owner paths, not reachable by any real user; a matching, already-accepted precedent exists from Module 3.8. Deleting them properly requires signing in as their throwaway owner accounts (Supabase's `storage.protect_delete()` trigger blocks direct SQL deletion) — not worth escalating to the service-role key for.
