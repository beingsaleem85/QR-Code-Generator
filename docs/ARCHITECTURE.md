...
- 5 small orphaned PDF blobs remain in the `qr-documents` Storage bucket from Journey F's runs (see Cleanup above) — harmless, isolated per-owner paths, consistent with the project's existing accepted-orphan precedent.
