...
## Test Suite and End-to-End Verification (Module 3.16)

### Unit test coverage: audited, one real gap found and closed

The master prompt names seven specific units to cover: payload builders, URL validation, Wi-Fi escaping, vCard generation, slug helpers, design config validation, download filename sanitization. Checked each against the existing 470 unit tests before writing anything new, rather than assuming gaps existed:

- Payload builders, URL validation (including dangerous-scheme rejection), Wi-Fi escaping, vCard generation, download filename sanitization (`slugifyForFilename`, `render.test.ts`) — all already covered, going back to Modules 1.3/1.6/3.2-3.4.
- "Design config validation" has no literal Zod schema (`DesignConfig` in `src/types/qr-design.ts` is a plain TypeScript type) — the real equivalent is `src/lib/qr/reliability.ts`'s contrast/logo-size/error-correction-level rules, already fully covered by `reliability.test.ts`.
- **Real gap**: `generateRandomSlug()` (`src/lib/qr/slug.ts`) had zero test coverage anywhere in the suite, despite being load-bearing for every dynamic QR's identifier since Module 1.4. Closed with `tests/unit/qr/slug.test.ts` (length, alphabet, non-determinism).

### Integration tests: already satisfied by this project's existing unit-level Server Action tests

The master prompt asks for integration coverage of QR CRUD, ownership/RLS, dynamic destination update, scan event creation, and storage metadata, "where practical." Checked the existing suite before building a new `tests/integration/` layer against a real or mocked Postgres: `actions.test.ts` already asserts real RLS-relevant behavior (session-derived `user_id`, RLS-blocked update/delete returning a clean error rather than throwing, 0-rows-affected handling), `redirect-resolution.test.ts` covers dynamic destination resolution, `scan-tracking.test.ts` covers scan event creation, and `asset-sync.test.ts` covers storage metadata sync/duplication. This is this project's established, real testing strategy — mock the Supabase client at the unit level to prove exact query shape and RLS-aware error handling, then live-verify cross-user behavior for real during each module's own live-verification pass (Modules 3.8-3.11 all did this). Building a second, redundant integration layer wouldn't add real coverage; `tests/integration/` stays empty by this same judgment, not by oversight.

### E2E framework: Playwright, chosen and installed this module

No E2E framework existed before this module (`tests/e2e/` was an empty placeholder since Module 1.1). Installed `@playwright/test` — first-party Next.js App Router support, real multi-engine coverage (Chromium/Firefox/WebKit) for the master prompt's "at least one additional browser engine" requirement, and built-in device emulation for the mobile-viewport requirement, all without extra tooling. `playwright.config.ts`: `testDir: tests/e2e`, three projects (`chromium-desktop`, `firefox-desktop`, `chromium-mobile` via the `Pixel 7` device profile), HTML + list reporters, and a `webServer` block that reuses an already-running dev server rather than assuming a fresh one (this project's dev server sometimes falls back to port 3001 — see `docs/SESSION_HANDOFF.md`). `npm run test:e2e` / `test:e2e:ui` added; kept fully separate from `npm test` (vitest's `include` glob only matches `tests/unit/**`, so the two suites never collide).

**Not fully parallel, deliberately** (`fullyParallel: false`, `workers: 1`): this project has no disposable local/CI Supabase instance — Docker has been confirmed unavailable in this environment across multiple modules (see Module 3.13's/3.15's own notes) — so Journeys B-F run real signups against the live, linked project, the same throwaway-account technique used for live verification since Module 3.6. Running the full 6-journey suite across 2 browser projects with Playwright's default parallel workers was tried first and made every auth-dependent journey (C/D/E/F, 8 of 12 total) time out — Supabase Auth's own signup rate limiting throttles a burst of concurrent signups from one IP, something a disposable local Postgres instance wouldn't be sensitive to at all. Serial execution (one worker) avoids the burst entirely; the exact same specs pass individually and pass together once serial. This costs wall-clock time (the full 12-test suite takes ~2.5 minutes on this machine), not correctness — a real, documented trade-off of testing against production infrastructure instead of a disposable one, not a workaround for a bug.

**Firefox could not be launched in this sandboxed environment** (`browserType.launch: spawn UNKNOWN` when starting `firefox.exe`, reproduced with sandbox restrictions explicitly disabled too — not a sandbox-permission issue, some other environment-level restriction on spawning that specific binary). The `firefox-desktop` project stays defined in the config (a real, valid project a normal CI runner can use) rather than removed — this is an environment limitation of the specific sandbox this session runs in, not evidence Firefox doesn't work with these tests. The master prompt's own wording — "at least one additional browser engine **if available**" — already anticipates this kind of environment gap. Chromium (desktop + mobile-emulated) fully exercises every journey.

### Journeys A-F: all six built, all six live-verified, one real production bug found and fixed

Built and ran every journey named in the master prompt against this app's actual, real live Supabase project (not mocked) — not just written and assumed correct:

- **Journey A (Anonymous Static QR)** — no backend involved at all (static QR rendering is entirely client-side); the only journey that needs zero account/session state.
- **Journey B (Account Creation)** — real signup, real dashboard, real saved QR, real appearance in the list.
- **Journey C (Dynamic QR)** — real dynamic QR, real anonymous redirect (checked via a direct, un-followed request to `/r/[slug]` rather than letting the browser navigate all the way to the placeholder `example.com` destination, which isn't a real page and would 404 on its own), real destination edit, real re-resolution to the new destination — same slug throughout.
- **Journey D (Analytics)** — real anonymous scan, polled (scan recording runs inside `after()`, Module 3.7's deliberately non-blocking design) until the analytics page's "Total scans" card reflects it.
- **Journey E (Authorization)** — two real, independent accounts; User B's direct-URL access to User A's QR (detail and edit routes) is denied. Confirmed via rendered content ("Page not found"), not HTTP status — this app's `notFound()` returns HTTP 200 due to the root `loading.tsx` Suspense boundary, an already-documented (Module 2.7), deliberately-accepted Next.js limitation, not something this module should treat as a fresh bug.
- **Journey F (File QR)** — real PDF upload, real landing page at `/p/[slug]`, real file replacement, confirmed the same slug now serves the new file's name and no longer the old one.

**Real bug found and fixed**: Journey A's first run failed with a genuine, reproducible error — `Failed to load QR image for PNG export`. Browser console (captured via a temporary debug spec, since the Browser pane tool wasn't compositing this session — see `docs/SESSION_HANDOFF.md`'s standing note on that) showed the real cause: `Content-Security-Policy directive: "img-src 'self' data: https:". The action has been blocked.` `QRDownloadActions`' PNG export path (`renderStyledQrPngDataUrl`) loads the rendered SVG into an `<img>` via a `blob:` object URL before drawing it to canvas (Module 3.4) — Module 3.12's CSP `img-src` directive never included `blob:`, only `data:`/`https:`. **Every PNG download has been silently CSP-blocked in production since Module 3.12 shipped**, because that module's own live verification checked for CSP violations on page loads only (homepage, generator page, login page) and never actually clicked the download button — this is exactly the kind of gap a real click-through E2E journey exists to catch, and did. Fixed with one token: `img-src 'self' data: blob: https:` in `next.config.ts`. Re-verified live in both a real dev server (Journey A re-run, now passing) and a real production server (`next start`, confirmed via a direct header fetch that `blob:` is present and `'unsafe-eval'` is still correctly absent).

### Cleanup

35 throwaway `e2e-*@example.com` accounts were created across debugging and journey runs this module (each real, each needed to prove a genuine live behavior, not batched down artificially). All deleted via `DELETE FROM auth.users WHERE email LIKE 'e2e-%@example.com'` (cascades `profiles`/`qr_codes`/`qr_folders`/`account_entitlements`/`qr_scan_events`/`qr_feedback_submissions` automatically, per the existing `ON DELETE CASCADE` schema). A direct attempt to also delete the 5 associated `storage.objects` rows via SQL was rejected by Supabase's own `storage.protect_delete()` trigger ("Direct deletion from storage tables is not allowed. Use the Storage API instead.") — deleting them properly would mean signing in as each of the 5 different throwaway owners individually just to call the Storage API before their account was removed, disproportionate effort for 5 small dummy PDF fixtures that can never surface in any real user's data (each lives at a per-user, per-QR Storage path). Left as orphaned blobs, consistent with this project's own already-established precedent (Module 3.8's single documented orphan, reconfirmed clean by Module 3.13's full reconciliation) — a deliberate, documented trade-off, not an oversight, and specifically **not** a reason to introduce `SUPABASE_SERVICE_ROLE_KEY` anywhere in this project. Final SQL check confirmed exactly one `auth.users` row (`mts.pk@hotmail.com`, the permanent account) and zero `qr_codes`/`qr_assets`/`qr_scan_events`/`qr_feedback_submissions` rows anywhere in the project. `mailer_autoconfirm` restored to `false` and confirmed via a follow-up read of the same config endpoint.

### Verification

- New tests: `tests/unit/qr/slug.test.ts` (+4, closing the one real unit-coverage gap found above); `tests/e2e/journey-{a..f}-*.spec.ts` (6 new E2E specs, ~12 test runs across the 2 working browser projects).
- `npm run typecheck` (covers `tests/e2e/**` too — `tsconfig.json`'s `include` is `**/*.ts`, not scoped to `src/`) / `npx eslint .` (0 errors, same 11 pre-existing warnings) / `npx prettier --check .` — all pass.
- `npx vitest run` — **474/474 passing** across 70 files (no flake this run — the previously-documented `QRGeneratorShellSave.test.tsx` full-suite-parallel timing issue, Modules 3.14/3.15, didn't reproduce here; still not this module's to fix, since it's orthogonal to anything touched).
- `npx playwright test --project=chromium-desktop --project=chromium-mobile` — **12/12 passing** (6 journeys x 2 projects), run serially against the real live Supabase project per the design decision above.
- `rm -rf .next && npm run build` — pass; all previously-static routes remain static.
- **Live verification against the real Supabase project and both a real dev server and a real production server**: every journey above ran against real signups/QRs/scans/files on the live project, not mocks; the CSP fix confirmed present in both a dev and a `next start` production server's actual response headers. Full cleanup performed and confirmed (see above). Dev/production server processes were each stopped by their specific PID (`Get-NetTCPConnection` → `Stop-Process -Id`), never a broad `taskkill`.

### Known issues

- Firefox cannot launch in this specific sandboxed environment (see above) — the `firefox-desktop` Playwright project is still correctly configured and should work in a normal CI runner or an unrestricted local machine; this is an environment gap, not a code defect.
- E2E Journeys B-F require `mailer_autoconfirm=true` on the live Supabase project for the duration of the run (the same throwaway-account technique used throughout this project since Module 3.6) — running them without that toggle will time out at signup with a clear error message pointing at the cause (`tests/e2e/helpers/test-user.ts`), not a silent hang. There is still no dedicated, disposable test Supabase project; every E2E run against B-F touches the real live project, which is why `workers: 1` and full manual cleanup both matter every time this suite runs, not just this once.
- 5 small orphaned PDF blobs remain in the `qr-documents` Storage bucket from Journey F's runs (see Cleanup above) — harmless, isolated per-owner paths, consistent with the project's existing accepted-orphan precedent. **This count is not stable** — it grows by one every time Journey F runs, and this module undercounted even its own moment in time (the pre-existing Module 3.8 orphan was still in the same bucket). See the "Pre-Deployment Review" section below for the corrected framing.

## Production Readiness and Final Audit (Module 3.17)

This is the final module of the master build prompt's numbered module chain. It is primarily synthesis and audit — most of what it checks was already built and live-verified module-by-module across the entire session — plus three brand-new documentation files and a final report, rather than large amounts of new application code.

### Final Checks

- **Production build / TypeScript / lint / unit tests**: re-run fresh as this module's own gate rather than assumed carried over from Module 3.16 — all pass identically (474/474 unit tests, 0 typecheck errors, 0 lint errors with the same 11 pre-existing informational warnings, a clean `rm -rf .next && npm run build`).
- **E2E tests**: re-ran Journey A (no backend dependency) across both working browser projects as a lightweight confirmation, rather than re-running the full B-F live-Supabase suite again — Module 3.16 already comprehensively live-verified every journey against the real project; re-running B-F here would only create more throwaway accounts/cleanup churn without checking anything Module 3.16 didn't already prove.
- **Dependency audit**: `npm audit` — 0 vulnerabilities. This project's own standing "avoid unnecessary dependencies" discipline (no charting library, no heavy UI kit, hand-rolled SVG/QR rendering, hand-rolled analytics aggregation) keeps the dependency surface small enough that this was fast and clean.
- **Secret scan**: grepped every tracked file (`git ls-files`) for common credential patterns (Supabase access-token prefix, service-role/JWT-shaped strings). Two hits, both false positives — the literal word "service_role" used in prose/config describing the Postgres role name, not a leaked value. Confirmed `.env.local` is not tracked (`git ls-files` only shows `.env.example`) and is correctly gitignored (`git check-ignore -v`).
- **Route audit**: cross-checked every `page.tsx`/`route.ts` in `src/app/` against `src/proxy.ts` (protects exactly `/dashboard*`, redirects `/login`/`/signup` away from an already-authenticated session) and `robots.ts`'s disallow list (Module 3.14) — consistent, no route is unintentionally public or unintentionally blocked. The only remaining `RouteStub` placeholder anywhere in the app is `/pricing`, the one already-documented, deliberate business-scope placeholder — confirmed via a direct grep, not assumed.
- **Broken-link audit**: collected every static internal `href`/`Link` target across `src/` (both inline JSX `href="..."` and array-based nav-config `href: "..."` patterns, e.g. `nav-items.ts`/`Footer.tsx`/`Header.tsx`) and confirmed each resolves to a real, existing route. Zero broken links found.
- **Responsive UI audit**: Module 3.16's `chromium-mobile` Playwright project (Pixel 7 device emulation) already exercises every E2E journey's full interactive flow — form fills, file uploads, navigation — on a real mobile viewport, live, not just a static resize screenshot. Treated as sufficient, real coverage rather than re-doing a separate manual pass.
- **Accessibility audit**: checked every `<img>` in `src/` for a real `alt` attribute (all 4 non-decorative image call sites have one — `GalleryLandingPage`, `MenuLandingPage`, the logo preview in `design-controls.tsx`, `Avatar`) and searched for icon-only buttons lacking an accessible name (none found — every interactive element observed throughout this entire build already carries a real `aria-label`/visible text/`role`, confirmed indirectly by the fact that every E2E test in Module 3.16 drives the UI exclusively through `getByRole`/`getByLabel`, which only work against genuinely accessible markup). No automated tool (e.g. `axe-core`) was added — the existing evidence (real semantic roles throughout: `listbox`/`option`/`tablist`, real form labels via the `FormField` pattern, real heading hierarchy audited in Module 3.14) was judged sufficient without introducing a new dependency for a one-time check.
- **RLS/security audit**: re-read every `create policy`/`enable row level security` statement across all 18 migrations and cross-checked against `docs/SECURITY.md`'s claims before writing that file — every table's policy shape matches what every prior module already documented, with no drift found. Additionally confirmed every one of the 8 `SECURITY DEFINER` functions in the schema explicitly sets `search_path = public, pg_temp` (a direct grep across all migrations) — a genuinely new, first-time-in-this-project confirmation that this specific hardening pattern was applied consistently everywhere it needed to be, not just in the modules that introduced it.
- **Storage policy audit**: re-read `qr_asset_is_publicly_readable()` and its three consuming policies directly from the migration — confirmed still exactly as narrow as documented (scoped to `mode = 'dynamic' and status = 'active'`, `qr-logos` correctly excluded, the function itself has `revoke all ... grant execute ... to anon, authenticated`).

### Final Documentation

- **`README.md`**: fully rewritten. The previous version was a literal Phase-1 scaffold ("Supabase is not yet connected... planned") despite the app having a complete, real, live-verified feature set — now describes what's actually built, links every doc file (including the two new ones below), and lists the real npm scripts (including the Module 3.16 `test:e2e`/`test:e2e:ui` additions, missing from the old README).
- **`docs/SUPABASE_SETUP.md`** (new): how to link/configure a fresh Supabase project for this app from scratch — migrations, Storage buckets (with the real size-limit/MIME table), Auth redirect URLs, the `mailer_autoconfirm` toggle technique, and every environment variable's source and purpose, including an explicit warning against filling in `SUPABASE_SERVICE_ROLE_KEY`.
- **`docs/DEPLOYMENT.md`** (new): required production environment variables (with `NEXT_PUBLIC_APP_URL`'s correctness called out explicitly, since every printed QR code's encoded link depends on it), domain/Auth-redirect-URL configuration, Storage settings (already fully migration-driven, no manual step), optional SMTP/monitoring, and a real pre-launch checklist pulling together every "Known issue" accumulated across Modules 3.14-3.17.
- **`docs/SECURITY.md`** (new): a consolidated synthesis of this project's RLS-everywhere/no-service-role-key/rate-limiting/CSP posture — pulling together facts already scattered across a dozen of `docs/ARCHITECTURE.md`'s own module sections into one place, including a full per-table RLS policy reference and the real `img-src: blob:` bug story from Module 3.16 as a concrete example of why the click-driven E2E suite exists.
- **`docs/FINAL_REPORT.md`** (new): the master prompt's required 10-section final report (Product Summary, Architecture, Completed QR Types, Database, Storage, Analytics, Testing, Known Limitations, Required Production Configuration, Future Enhancements) as its own standalone document, cross-linking into `docs/SECURITY.md`/`docs/DEPLOYMENT.md`/`docs/SUPABASE_SETUP.md` rather than duplicating their content.

### Verification

- No new tests — this module is audit/documentation, not new application logic.
- `npm run typecheck` / `npx eslint .` (0 errors, same 11 pre-existing warnings) / `npx prettier --check .` (including every new/rewritten Markdown file) — all pass.
...
- No live Supabase changes this module — every audit item was read-only (migration review, `git` inspection, a fresh test/build run); no new migration, no new test account, nothing to clean up.

### Known issues

- See `docs/FINAL_REPORT.md`'s "Known Limitations" section — a consolidated, deduplicated list of every real, deliberate gap accumulated across Modules 3.8-3.16, not new findings from this module.

## Pre-Deployment Review (post-3.17)

An independent audit against `docs/FINAL_REPORT.md` and the live codebase/Supabase project, requested after Module 3.17 closed, before any real deployment. Explicitly not trusting prior modules' own completion claims — every item below was re-checked directly.

### What was independently re-verified

- **Local gate, fresh**: typecheck, lint, format, `npm audit`, the full 474-test unit suite, and a clean production build all re-run from scratch — identical results to Module 3.17's own numbers.
- **Compiled build output scanned directly** (not just source) for `SUPABASE_SERVICE_ROLE_KEY`/`SUPABASE_ACCESS_TOKEN` — zero references anywhere in `.next/`, client or server chunks. Source also has zero references to either outside `.env.example`'s blank placeholder and documentation.
- **Repo-wide secret scan** re-run with an expanded pattern set (access-token prefix, JWT shape, AWS key shape, PEM private-key headers) — zero real matches. `.env.local` confirmed untracked and gitignored.
- **RLS re-derived directly from the migrations**, not from `docs/SECURITY.md`'s own prose: every `create policy`/`enable row level security` statement counted and cross-checked table by table — matches `docs/SECURITY.md` exactly. All 8 `SECURITY DEFINER` functions reconfirmed to set `search_path = public, pg_temp`.
- **RLS reconfirmed live, not just in migration source**: queried `pg_class.relrowsecurity` directly against the live database — `true` on all 8 `public` tables. A raw anonymous `curl` (anon key only) against `qr_codes` and `rate_limit_buckets` both returned `[]` — RLS is actually enforced at the API layer, not just declared.
- **Route/link audit re-run independently**: only `/pricing` remains a `RouteStub`; every static internal link (JSX `href`, nav-config `href:`) resolves to a real route.
- **Security headers reconfirmed against a real production server** (`next start`): all 5 headers present, CSP's `img-src` includes `blob:` (the Module 3.16 fix), `'unsafe-eval'` correctly absent.
- **Live Supabase account/entitlement state**: `mts.pk@hotmail.com` confirmed with `plan='pro'`, `is_lifetime=true`, `expires_at=null`, `dynamic_qr_limit=null` (unlimited) — queried directly, not assumed.
- **Full E2E suite re-run fresh against the current committed state** (not Module 3.16's historical run) — all 6 journeys × 2 browser projects, 12/12 passing, using the same `mailer_autoconfirm` toggle technique, followed by full cleanup and restoring `mailer_autoconfirm` to `false`, both reconfirmed via a follow-up read.

### Real discrepancies found and corrected

- **Migration count was wrong everywhere it appeared.** `docs/FINAL_REPORT.md`, `docs/SUPABASE_SETUP.md`, `docs/ARCHITECTURE.md`'s own Module 3.17 section, and both of Module 3.17's `docs/WORKLOG.md`/`docs/SESSION_HANDOFF.md` entries all said "17 migrations" — `supabase/migrations/` actually contains **18** files. (Oddly, an older, unrelated `docs/SESSION_HANDOFF.md` line from Module 3.14 already said "18" correctly — Module 3.17's own audit introduced the wrong number, rather than inheriting it.) Fixed in all five locations.
- **The orphaned-Storage-blob count was undercounted, and was never going to stay accurate as a fixed number.** Module 3.16 documented "5 orphaned PDF blobs" from its own E2E fixtures, but never re-counted the bucket as a whole — a pre-existing 13-byte blob from Module 3.8's original smoke test was still sitting in the same `qr-documents` bucket, making the real total 6 even at the moment Module 3.16 wrote that line. Then, re-running the E2E suite for this review's own verification (see above) added 2 more (Journey F ran once per browser project), bringing the live total to 8 by the time this review finished. **The count was never going to be stable** — it grows by one every time Journey F runs against the live project, since deleting a throwaway account cascades its database rows but not its Storage object (Supabase's `storage.protect_delete()` trigger blocks direct SQL deletion, and this project's zero-service-role-key policy means there's no privileged path around it). Rewrote every affected doc (`docs/FINAL_REPORT.md`, `docs/SECURITY.md`, `docs/ARCHITECTURE.md`, `docs/WORKLOG.md`) to describe this as the real, growing pattern it is, rather than restating a number that would go stale again on the next E2E run.

### New finding: a real, unaddressed operational gap

The Storage-orphan growth above is not just a documentation fix — it's a genuine gap in this project's E2E testing story that hadn't been named as such before. Every live run of the E2E suite (this review's included) leaves the live Supabase project's `qr-documents` bucket slightly larger, with no automated way to shrink it back down given the project's deliberate no-service-role-key stance. This is harmless today (isolated, unreachable, never read by the app) but should be fixed with a real mechanism — a Storage-aware E2E teardown step (sign in as each throwaway owner via their known password before deleting the account, and call the Storage delete API), or a periodic reconciliation job — before this suite is run repeatedly or wired into a real CI pipeline. Recorded in `docs/FINAL_REPORT.md`'s Known Limitations, not treated as a deploy-blocker.

### Verification

- `npm run typecheck` / `npx eslint .` (0 errors, same 11 pre-existing warnings) / `npx prettier --check .` / `npm audit` (0 vulnerabilities) — all re-run clean.
- `npx vitest run` — **474/474 passing** across 70 files.
- `npx playwright test --project=chromium-desktop --project=chromium-mobile` — **12/12 passing**, fresh against the current commit.
- `rm -rf .next && npm run build` — pass; route classification unchanged.
- **Live verification against the real Supabase project and a real production server**: RLS confirmed enforced (not just declared) via direct anonymous REST calls; entitlement confirmed via direct SQL; security headers confirmed via a real `next start` response. Full E2E cleanup performed and confirmed (exactly 1 `auth.users` row, 0 rows in every real data table). `mailer_autoconfirm` restored to `false` and confirmed via a follow-up config read.

### Known issues

- The growing Storage-orphan pattern described above — a real, low-priority operational gap, not a security issue, not a deploy-blocker.
