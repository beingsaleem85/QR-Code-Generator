...
- **Small orphaned PDF blobs accumulate in `qr-documents` every time the E2E suite's File QR journey runs against the live project** (8 present as of the pre-deployment review pass: 1 pre-existing 13-byte blob from Module 3.8's original smoke test, plus one per E2E run's Journey F account, since deleting a throwaway account cascades its database rows but not its Storage objects — see `docs/SECURITY.md`). Harmless, isolated per-owner paths, unreachable by any real user, and never counted or read by the app. Not a fixed number — it grows by one object each time Journey F runs, and shrinks only through a manual cleanup that signs in as each owner account (Supabase's `storage.protect_delete()` trigger blocks direct SQL deletion) — not worth escalating to the service-role key for. Worth a real automated fix (a Storage-aware E2E teardown, or a scheduled reconciliation job) before running this suite repeatedly/in CI against the live project.
